...
- **Metal GPU rendering: in progress, kernel pipeline now fully wired and building.** Added `PF_OutFlag2_SUPPORTS_GPU_RENDER_F32` in `GlobalSetup`, plus `GPUDeviceSetup` (`PF_Cmd_GPU_DEVICE_SETUP`), `GPUDeviceSetdown` (`PF_Cmd_GPU_DEVICE_SETDOWN`), and `SmartRenderGPU` (`PF_Cmd_SMART_RENDER_GPU`), dispatched from `EffectMain`.
  ...
  - **Intermediate GPU world allocation: DONE.** Added a `PF_GPUDeviceSuite1::CreateGPUWorld`/`DisposeGPUWorld` pair in `SmartRenderGPU` to allocate the horizontal-pass scratch buffer (GPU equivalent of `DoBlur()`'s `PF_WorldSuite1::new_world` scratch world, which is CPU-only). Real signatures confirmed via Xcode jump-to-definition: `DisposeGPUWorld(effect_ref, worldP)` (2 args, matched the first guess); `CreateGPUWorld(effect_ref, device_index, width, height, PF_RationalScale pixel_aspect_ratio, PF_Field field_type, PF_PixelFormat pixel_format, PF_Boolean clear_pixB, PF_EffectWorld **worldPP)` (9 args, needed 5 more than the first guess). Wired using `in_data->pixel_aspect_ratio`, `PF_Field_FRAME`, and `FALSE` for `clear_pixB` (skipped — the blur pass overwrites every pixel anyway). `PF_PixelFormat_GPU_BGRA128` confirmed real (`MAKE_PIXEL_FORMAT_FOURCC('@','C','D','A')`, GPU/BGRA/32-bit float per channel). Also confirmed via the full `AcquireExclusiveDeviceAccess` doc comment that exclusive-access calls aren't needed here — that's only for plugins sharing one render path between CPU and GPU, and OmniBlur has a separate `SmartRenderGPU` entry point.
  - **Real dispatch: DONE, tested and confirmed working in AE.** Read the actual generated Metal kernel signature by temporarily dumping the compiled source to disk (now removed): `kernel void BoxBlurKernel(device const float4* inSrc [[buffer(0)]], device float4* outDst [[buffer(1)]], device BoxBlurKernelValues *inValues [[buffer(2)]], uint2 inXY [[thread_position_in_grid]])`, where `BoxBlurKernelValues` packs `inSrcPitch, inDstPitch, in16f, inWidth, inHeight, inRadius, inHorizontal` — notably including an `in16f` field the earlier placeholder sketch didn't have. Wrote `SmartRenderGPU`'s dispatch to match: `GetGPUWorldData` pulls each world's real GPU buffer (bridged to `id<MTLBuffer>`, same `__bridge` pattern as `devicePV`/`command_queuePV`), then two `dispatchThreads` passes on one command buffer/encoder (horizontal input→intermediate, vertical intermediate→output), with a `memoryBarrierWithScope:MTLBarrierScopeBuffers` between passes since Metal doesn't auto-sync successive dispatches within one encoder. Pitch converted from `rowbytes` (bytes) to a float4-element stride via `/ 16`. **Clean build (build 15) and confirmed visually blurring correctly in AE** — first genuinely working GPU-accelerated render.
- **Metal GPU rendering: functionally complete end-to-end** — kernel generation, device lookup, pipeline creation, intermediate buffer, and dispatch are all wired and confirmed working. Remaining work on this path is polish/robustness, not new functionality: delete the dead "Files '.cl' using Script" Build Rule (still not done), and revisit the handful of `VERIFY`-tagged assumptions that happened to work on the first real test but weren't independently confirmed — `in16f = 0` being the correct "not half-float" value, and `extra->input->device_index` existing on `PF_SmartRenderExtra` the same way it does on `GPUDeviceSetupExtra`.
...
- Current build: v1.0.3, build 15 (bump before next test build)
